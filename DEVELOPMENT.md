...
* 提供一个简单的RESTful的WebAPI（`flask_app.py`）
...
* WebAPI
    * 支持新建/更新事件时使用其他的时间类型
    * 支持「集合新增」以一次新增许多项目，且此堆项目中可以互相使用别名而非ID进行引用
    * 支持对每个数据库修改动作进行完备性和一致性检查，并在为否时阻止执行动作（可以临时新建一个数据集，包含原有数据和新数据，检查）
    * 支持自定义数据集，选取部分记录组合成一个（临时？）数据集
...
- 主要模块：
    - `inforec.py`：CLI入口
    - `flask_app.py`：WebAPI程序入口（并及未来WebUI入口）
    - `model.py`：
...
使用Flask实现，细节参见`flask_app.py`。

目前已实现新增、查询、更新**事件**（通过`/api/event`和`/api/event/<id>`两个入口），其可支持相对另一事件的相对时间。

支持列出整个数据集的全部事件。
另支持对整个数据集合的完备性和一致性检查，以及列出所有的冲突项。
